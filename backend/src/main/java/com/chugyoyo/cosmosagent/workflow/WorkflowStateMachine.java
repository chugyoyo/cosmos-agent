...
public class WorkflowStateMachine {
    ...
    private Map<String, Object> executeLLMNode(WorkflowContext context, AgentNode node, Map<String, Object> input) {
        try {
            ...
            String outputVariable = (String) llmConfig.getOrDefault("outputVariable", "llmOutput");
            Double temperature = Double.parseDouble(String.valueOf(llmConfig.getOrDefault("temperature", "0.7")));
            Integer maxTokens = Integer.parseInt(String.valueOf(llmConfig.getOrDefault("maxTokens", "1000")));
            ...
            output.put(outputVariable, response.getContent()); // 添加text字段，默认使用 LLM 响应内容
            output.put("timestamp", System.currentTimeMillis());
